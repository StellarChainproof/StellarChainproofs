import type { BridgeAnalysisReport, BridgeFinding } from "./types";

/** Deterministic, recursively key-sorted JSON suitable for versioned CI artifacts. */
export function serializeBridgeReport(report: BridgeAnalysisReport): string {
  return JSON.stringify(sortValue(report), null, 2) + "\n";
}

/** Human-readable bridge report with evidence and explicit scope limitations. */
export function generateBridgeMarkdown(report: BridgeAnalysisReport): string {
  const lines = [
    ...
    `- Bridge contracts modeled: ${report.summary.contracts}`,
  ...
  for (const file of report.files) {
    ...
    if (!file.findings.length) lines.push("No structural bridge findings.", "");
    for (const finding of file.findings) appendFinding(lines, finding);
  }
  lines.push(
    "## Scope",
    "",
    "This report evaluates cross-chain bridge safety: message verification, domain separation, replay protection, " +
...
function appendFinding(lines: string[], finding: BridgeFinding): void {
  lines.push(
