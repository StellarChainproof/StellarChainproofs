...
const RULE_PATTERN = /^CP-BRG-(?:00[1-9]|01[0-6])$/;
...
export function registerBridgeCommand(program: Command, printBanner: () => void): void {
  program
    .command("bridge <targets...>")
    .description("Analyze cross-chain bridge and message verification safety")
    .option("--format <format>", "Output format: json|markdown", "markdown")
    .option("--output <file>", "Write the report to a file")
    .option("--config <file>", "Load a versioned bridge analysis configuration")
    .option("--include-models", "Include the normalized bridge model in JSON output")
    ...
    .action((targets: string[], raw: BridgeCliOptions) => {
      ...
      try {
        ...
        const configured = raw.config ? loadBridgeConfigFile(raw.config) : undefined;
        ...
        const options: BridgeAnalysisOptions = {
          limits,
          includeModels: raw.includeModels ?? configured?.config.includeModels ?? false,
          ...(includeRules ? { includeRules } : {}),
          ...(excludeRules ? { excludeRules } : {}),
        };
        const report = analyzeBridgeFiles(targets, options);
        const output = raw.format === "json"
          ? serializeBridgeReport(report)
          : generateBridgeMarkdown(report);
        if (raw.output) {
          writeReport(raw.output, output);
          if (!json) console.log(chalk.green(`\n  Bridge report written to ${raw.output}`));
        } else {
          process.stdout.write(output);
        }
        process.exit(exitCode(report, raw.failOn));
      } catch (error) {
        const message = error instanceof BridgeConfigError ||
          error instanceof BridgeAnalysisCancelledError || error instanceof Error
          ? error.message
          : "Bridge analysis failed";
        console.error(chalk.red(`Bridge analysis error: ${sanitize(message)}`));
        process.exit(2);
      }
    });
}
...
function positiveInteger(value: string): number {
  if (!/^\d+$/.test(value)) throw new BridgeConfigError("analysis limits must be positive integers");
  const result = Number(value);
  if (!Number.isSafeInteger(result) || result <= 0) {
    throw new BridgeConfigError("analysis limits must be positive safe integers");
  }
  return result;
}

function validateRules(values: string[], option: string): BridgeRuleId[] {
  const result = new Set<BridgeRuleId>();
  for (const value of values) {
    if (!RULE_PATTERN.test(value)) throw new BridgeConfigError(`${option} contains unknown rule ${value}`);
    result.add(value as BridgeRuleId);
  }
  return [...result].sort();
}

function rejectOverlap(include: BridgeRuleId[] | undefined, exclude: BridgeRuleId[] | undefined): void {
  if (!include || !exclude) return;
  const overlap = include.filter((rule) => exclude.includes(rule));
  if (overlap.length) throw new BridgeConfigError(`included and excluded rules overlap: ${overlap.join(", ")}`);
}

function validateFormat(value: string): asserts value is OutputFormat {
  if (value !== "json" && value !== "markdown") {
    throw new BridgeConfigError("--format must be json or markdown");
  }
}

function validateFailSeverity(value: string): asserts value is FailSeverity {
  if (!(value in SEVERITY_RANK)) {
    throw new BridgeConfigError("--fail-on must be none, info, low, medium, high, or critical");
  }
}

function exitCode(report: BridgeAnalysisReport, threshold: FailSeverity): number {
...
function writeReport(file: string, output: string): void {
  try {
    ...
    throw new BridgeConfigError(`report file could not be written (${safeCode})`);
  }
}
